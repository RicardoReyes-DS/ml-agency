...
import { ShaderMaterial, Vector2, Mesh, PlaneGeometry, WebGLRenderer } from "three";
import type { DomainColoringProps, ComplexFunctionType, ColorMode } from "./domain-coloring-canvas";
...
// Own the asynchronous renderer initialization so failures cannot escape as
// unhandled configure() rejections outside the local React error boundary.
export function DomainColoringScene({
  functionType = "transfer", colorMode = "monochrome", speed = 0.5,
  opacity = 0.4, mouseInfluence = 0.3, colorShift = 0, zoom = 1.0,
  onUnavailable,
}: DomainColoringProps & { onUnavailable: () => void }) {
  ...
  useEffect(() => {
    ...
    const cleanup = () => {
      ...
      root?.unmount(); // Stops R3F's frame loop and disposes scene geometry/materials.
      renderer?.dispose();
    ...
    try {
      ...
      const gl = renderer;
      // Serialize resize/configure to avoid competing renderer initialization.
      ...
      const resize = () => {
        pending = pending.then(async () => {
          ...
          await sceneRoot.configure({
            gl, events, camera: { position: [0, 0, 1], fov: 75 },
            size: { width, height, top, left }, dpr: [1, 1.5],
            onCreated: (state) => {
              canvas.dataset.domainReady = "true";
              state.events.connect?.(canvas.closest<HTMLElement>("section, footer") ?? canvas);
            },
          });
          if (disposed) {
            sceneRoot.unmount(); gl.dispose(); gl.forceContextLoss();
            return;
          }
          sceneRoot.render(
            <SceneBoundary onUnavailable={onUnavailable}>
              <DomainColoringMesh
                functionType={functionType} colorMode={colorMode} speed={speed}
                opacity={opacity} mouseInfluence={mouseInfluence} colorShift={colorShift} zoom={zoom}
              />
            </SceneBoundary>
          );
    ...
    return cleanup;
  }, [functionType, colorMode, speed, opacity, mouseInfluence, colorShift, zoom, onUnavailable]);

  return <canvas ref={canvasRef} data-domain-scene="" className="absolute inset-0 w-full h-full" />;
}

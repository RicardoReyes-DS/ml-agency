...
import { AlertCircle, Bot, FileText, Loader2, Send, Upload, User, X } from "lucide-react";
...
const copyByLocale = {
  es: {
    fileTooLarge: "El archivo excede el limite de 10MB.",
    initialMessage: 'Ya procese "{filename}". Ahora puedes preguntarme sobre su contenido.',
    processingError: "Ocurrio un error inesperado durante el procesamiento.",
    responseError: "Lo siento, ocurri? un error al generar la respuesta.",
    processing: "Procesando documento...",
    processingHint: "Extrayendo texto para analisis",
    uploadTitle: "Sube una base de conocimiento",
    uploadDescription:
    ...
    ready: "Contexto cargado • Listo para preguntas",
    clearContext: "Limpiar contexto",
    placeholder: "Haz una pregunta sobre el documento...",
  },
  en: {
    fileTooLarge: "File size exceeds 10MB limit.",
    initialMessage: 'I\'ve processed "{filename}". You can now ask me questions about its content.',
    processingError: "An unexpected error occurred during processing.",
    responseError: "Sorry, I encountered an error generating the response.",
    processing: "Processing document...",
    processingHint: "Extracting text for analysis",
    uploadTitle: "Upload Knowledge Base",
    uploadDescription:
      "Drag & drop a PDF, DOCX, Image, or Text file here, or click to select.",
    uploadLimit: "(Max 10MB)",
    selectDocument: "Select Document",
    ready: "Context loaded • Ready for questions",
    clearContext: "Clear Context",
    placeholder: "Ask a question about the document...",
  },
} as const;

export function RAGInterface({ className, locale = "en" }: RAGInterfaceProps) {
    ...
    try {
      ...
      if (result.error) {
        ...
        setChatHistory([
          {
            role: "model" as const,
            parts: [{ text: copy.initialMessage.replace("{filename}", result.filename) }],
          },
        ]);
      }
    } catch (err) {
      setUploadError(copy.processingError);
      console.error(err);
      setFile(null);
    } finally {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    maxFiles: 1,
    disabled: isProcessing || !!extractedContext,
  });
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setChatHistory((prev) => [
        ...prev,
        { role: "model" as const, parts: [{ text: copy.responseError }] },
      ]);
    } finally {
          ...
          >
            ...
            {isProcessing ? (
              <div className="flex flex-col items-center space-y-4">
                ...
                <p className="text-lg font-medium">{copy.processing}</p>
                <p className="text-sm text-muted-foreground">{copy.processingHint}</p>
              </div>
            ) : (
              <>
                ...
                <h3 className="text-xl font-semibold mb-2">{copy.uploadTitle}</h3>
                <p className="text-muted-foreground max-w-md mx-auto mb-6">
                  {copy.uploadDescription}
                  <br />
                  <span className="text-xs text-muted-foreground/70">{copy.uploadLimit}</span>
                </p>
          ...
          <CardHeader className="border-b border-primary/10 bg-surface/50 p-4 flex flex-row items-center justify-between space-y-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription className="text-xs">{copy.ready}</CardDescription>
              </div>
            </div>
